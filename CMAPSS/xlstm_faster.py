...
#KAN(width=[int(p_factor * inp_dim),hid_dim], grid=10, k=5, seed=0)
class KanBlockDiagonal(nn.Module):
    def __init__(self, in_features, out_features, num_blocks):
        super(KanBlockDiagonal, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.num_blocks = num_blocks
        ...
        self.blocks = nn.ModuleList([
            KAN(layers_hidden=[self.in_features,block_out_features], num_grids=5)
            for _ in range(num_blocks)
        ])
...
class KANmLSTMBlock(nn.Module):
    def __init__(self, input_size, head_size, num_heads, proj_factor=2):
        super(KANmLSTMBlock, self).__init__()
        self.input_size = input_size
        self.head_size = head_size
        self.hidden_size = head_size * num_heads
        self.num_heads = num_heads
        ...
        self.Wq = KanBlockDiagonal(int(input_size * proj_factor), self.hidden_size, num_heads)
        self.Wk = KanBlockDiagonal(int(input_size * proj_factor), self.hidden_size, num_heads)
        self.Wv = KanBlockDiagonal(int(input_size * proj_factor), self.hidden_size, num_heads)
    ...
    def __init__(self, input_size, head_size, num_heads, layers, batch_first=False, proj_factor_slstm=4/3, proj_factor_mlstm=2):
        ...
        for layer_type in layers:
            if layer_type == 's':
                ...
                layer = KANmLSTMBlock(input_size, head_size, num_heads, proj_factor_mlstm)
            else:
...
if __name__ == '__main__':
    ...
    model = KANxLSTM(50, 8, 2, "ms", batch_first=False, proj_factor_slstm=4/3, proj_factor_mlstm=2)
    # random input tensor
    inp = torch.rand(30, 128, 50)    # 128 samples, 30 time steps, 50 features  
    ...
    print(out.shape)
    model = xLSTM(50, 8, 2, "ms", batch_first=False, proj_factor_slstm=4/3, proj_factor_mlstm=2)
    start = time.time()
    out, hid = model(inp)
    end_time = time.time()
    print(f"lstm代码运行时间: {elapsed_time} 秒")
    print(out.shape)
    mem_usage = get_model_memory_usage(model)
    print(f"模型内存占用: {mem_usage/1024} M")
    print(f"模型的参数数量: {count_parameters(model)} 个")
